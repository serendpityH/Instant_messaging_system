      ...
      ws.on('message', async (message) => {
        try {
          ...
          if (data.type === 'message') {
            try {
              // 验证消息格式
              if (!data.content || !data.content.receiverKey || !data.content.senderKey) {
              ...
              const newMessage = new Message({
                ...
                iv: data.content.iv,
                receiverKey: data.content.receiverKey,  // 接收者的加密密钥
                senderKey: data.content.senderKey,      // 发送者的加密密钥
              ...
              wss.clients.forEach((client) => {
                if (client.userId === data.receiver && client.readyState === WebSocket.OPEN) {
                  client.send(JSON.stringify({
                    ...
                    sender: ws.userId,
                    content: {
                      key: data.content.receiverKey,  // 只发送接收者的密钥
                      iv: data.content.iv,
                      content: data.content.content,
                      signature: data.content.signature
                    },
