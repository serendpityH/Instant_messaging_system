...
router.get('/:groupId/members', auth, async (req, res) => {
  try {
    ...
    if (!isMember) {
      return res.status(403).json({ message: '没有权限查看成员列表' });
    }
...
router.delete('/:groupId/members/:userId', auth, async (req, res) => {
  try {
    ...
    if (req.params.userId === req.userId) {
      return res.status(400).json({ message: '群主不能被移除' });
    }
    ...
    res.status(500).json({ message: '移除群组成员失败' });
  }
});
...
router.get('/:groupId/messages', auth, async (req, res) => {
  try {
    ...
    console.log('数据库中的消息:', messages);

    // 格式化消息
    const formattedMessages = messages.map(msg => ({
      sender: msg.sender._id,
      senderName: msg.senderName,
      content: msg.content,    // 加密的消息内容
      iv: msg.iv,             // IV
      createdAt: msg.createdAt
    }));

    console.log('发送给客户端的消息:', formattedMessages);
