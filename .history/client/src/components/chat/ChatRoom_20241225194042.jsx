...
const ChatRoom = () => {
  ...
  const initializeChat = async () => {
    try {
      ...
      // 3. 设置密钥到 cryptoService
      cryptoService.setPrivateKey(privateKey);
      cryptoService.setMyPublicKey(publicKey);
      ...
      // 4. 获取用户列表和初始化 WebSocket
      await fetchUsers();
      initializeWebSocket();
    } catch (error) {
      console.error('初始化聊天失败:', error);
      if (error.message.includes('未找到密钥')) {
        navigate('/login');
      } else {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      handleError('发送消息失��: ' + error.message);
    }
  };
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || '获取当前用户信息失败');
      }
