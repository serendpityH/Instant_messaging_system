...
const ChatRoom = () => {
  ...
  const handlePrivateMessage = async (message) => {
    try {
      ...
      handleError('处理消息失���: ' + error.message);
    }
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      let message;
      if (selectedUser) {
        console.log('准备发送私聊消息给:', selectedUser.username);
        
        // 私聊消息加密
        const encryptedContent = cryptoService.encryptMessage(newMessage, selectedUser._id);
        console.log('消息加密结果:', {
          hasReceiverKey: !!encryptedContent.receiverKey,
          hasSenderKey: !!encryptedContent.senderKey,
          hasContent: !!encryptedContent.content,
          hasIV: !!encryptedContent.iv,
          hasSignature: !!encryptedContent.signature
        });
        
        message = {
          type: 'message',
          receiver: selectedUser._id,
          content: encryptedContent  // 直接传递整个加密内容对象
        };
  ...
  const handleUserSelect = async (user) => {
    try {
      ...
      if (!user) {
        console.error('选择的用户为空');
        return;
      }

      setSelectedUser(user);
      setSelectedGroup(null); // 清空选中的群组
      setMessages([]); // 清空消息列表
      
      // 加载私聊消息
      console.log('准备加载私聊消息...');
      await loadMessages();

    } catch (error) {
      console.error('选择用户失败:', error);
      handleError('选择用户失败: ' + error.message);
    }
  };
      ...
      if (selectedGroup) {
        ...
        const decryptedMessages = messages.map(msg => {
          try {
            ...
            // 获取发送者的公钥
            const senderPublicKey = users.find(u => u._id === msg.sender)?.publicKey;
            if (!senderPublicKey) {
              throw new Error('找不到发送者的公钥');
            }
