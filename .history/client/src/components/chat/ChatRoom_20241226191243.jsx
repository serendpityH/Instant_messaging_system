  ...
  return (
    <div className="chat-container">
      ...
      <div className="nav-sidebar">
        ...
        <div 
          className={`nav-item ${showGroupList ? 'active' : ''}`}
          onClick={() => {
        ...
        <div 
          className={`nav-item ${showUserList ? 'active' : ''}`}
          onClick={() => {
        ...
        {(selectedUser || selectedGroup) && (
          <>
            <div className="chat-header">
              <h3>{selectedUser?.username || selectedGroup?.name}</h3>
              {selectedGroup && (
                <div className="header-actions">
                  <button onClick={() => setShowMembers(true)}>成员管理</button>
                  {selectedGroup.members.some(m => 
                    m.user === currentUser?._id && m.role === 'owner'
                  ) && (
                    <button onClick={() => setShowPendingMembers(true)}>
                      审核申请
                    </button>
                  )}
                </div>
