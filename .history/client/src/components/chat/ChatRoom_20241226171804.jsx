...
const ChatRoom = () => {
  ...
  const loadMessages = async () => {
    try {
      ...
      if (selectedGroup) {
        ...
        console.log('所有消息解���完成:', decryptedMessages);
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        handleError({ type: 'success', message: '密码修改成功，请重新登录' });
        setTimeout(() => {
          handleLogout();
        }, 2000);
      } else {
        handleError(data.message || '密码修改失败');
      }
    } catch (error) {
      console.error('修改密码失败:', error);
      handleError('修改密码失败，请稍后重试');
    }
  };
