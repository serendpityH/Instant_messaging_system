...
const ChatRoom = () => {
  ...
  const handlePrivateMessage = async (message) => {
    try {
      console.log('处理接收到的私聊消息:', {
        sender: message.sender,
        content: {
          hasKey: !!message.content.key,
          hasIv: !!message.content.iv,
          hasContent: !!message.content.content,
          hasSignature: !!message.content.signature
        }
      });
      ...
      console.log('消息解密完成:', {
        hasResult: !!decryptedContent,
        length: decryptedContent ? decryptedContent.length : 0
      });
    ...
    try {
      ...
      const processedMessages = await Promise.all(historicalMessages.map(async msg => {
        try {
          ...
          }, msg.sender === currentUser._id ? currentUser.publicKey : user.publicKey);

          console.log('历史消息解密结果:', {
            messageId: msg._id,
            decryptedContent
          });
          ...
          return {
            _id: msg._id,
            sender: msg.sender,
            content: decryptedContent,  // 直接使用解密后的文本
