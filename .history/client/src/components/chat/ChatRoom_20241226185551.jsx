...
const ChatRoom = () => {
  ...
  const handleGroupSelect = async (group) => {
    try {
      // 获取完整的群组信息，包括成员列表
      const response = await fetch(api.groups.detail(group._id), {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      // 加载群组消息
      const messagesResponse = await fetch(api.groups.messages(group._id), {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      const historicalMessages = await messagesResponse.json();
  ...
  const loadMessages = async () => {
    try {
      ...
      if (selectedGroup) {
        console.log('加载群���消息:', selectedGroup._id);
              ...
              {groupMembers.map(member => (
                <div key={member.user._id} className="member-item">
                  ...
                  {selectedGroup.members.some(m => 
                    ...
                    <button onClick={() => handleRemoveMember(selectedGroup._id, member.user._id)}>
                      移除
                    </button>
                  )}
                </div>
