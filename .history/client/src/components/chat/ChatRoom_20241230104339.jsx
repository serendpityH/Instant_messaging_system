...
const ChatRoom = () => {
  ...
  const initializeWebSocket = () => {
      ...
      try {
        ...
        switch (message.type) {
          ...
          case 'onlineUsers':
            ...
            handleOnlineUsersList(message.users);
            break;
          case 'heartbeat_ack':
            console.log('收到心跳响应');
            break;
    ...
    ws.current.onclose = (event) => {
      console.log('WebSocket连接已断开，正在重新连接...', event);
      // 清除在线用户列表
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('获取到的用户列表:', usersList.map(u => ({
          username: u.username,
          isOnline: u.isOnline
        })));
        ...
        handleError(error.message || '获取用户列表失败');
      }
    } catch (error) {
  ...
  const handleOnlineUsersList = (userIds) => {
    ...
    setUsers(prevUsers => {
      ...
      console.log('更新后的用户列表:', updatedUsers.map(u => ({
        username: u.username,
        isOnline: u.isOnline
      })));
      return updatedUsers;
  ...
  const handleUserStatusChange = (message) => {
    ...
    setUsers(prevUsers => {
      ...
      console.log('状态变化后的用户列表:', updatedUsers.map(u => ({
        username: u.username,
        isOnline: u.isOnline
      })));
      return updatedUsers;
        ...
        <div className="list-sidebar">
          ...
          <div className="users-content">
            <div className="users-list">
              <div className="list-title">用户列表</div>
              {filteredUsers.map(user => (
                ...
                >
                  ...
                  <div className="item-info">
                    ...
                    <div className={`item-status ${user.isOnline ? 'online' : 'offline'}`}>
                      {user.isOnline ? '在线' : '离线'}
                    </div>
                  </div>
                </div>
