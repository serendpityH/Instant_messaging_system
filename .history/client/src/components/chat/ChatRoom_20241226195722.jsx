...
const ChatRoom = () => {
  ...
  const handleGroupSelect = async (group) => {
    try {
      ...
      // 获取完整的群组信息
      const response = await fetch(`${api.groups.base}/${group._id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      console.log('获取到的群组详细信息:', {
        groupId: groupDetail._id,
        groupName: groupDetail.name,
        membersCount: groupDetail.members?.length,
      ...
      handleError('加载群组信息失败: ' + error.message);
    }
  };
  ...
  const handleDisbandGroup = async (groupId) => {
    if (!window.confirm('确定要解��该群聊吗？此操作不可恢复！')) return;
  ...
  const loadMessages = async () => {
    try {
      ...
      if (selectedGroup) {
        ...
        response = await fetch(api.groups.messages(selectedGroup._id), {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
        ...
        response = await fetch(api.messages.list(selectedUser._id), {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
            ...
            {selectedGroup && (
              <div className="header-actions">
                <button 
                  ...
                  onClick={() => {
                    console.log('点击成员列表按钮');
                    console.log('当前群组信息:', selectedGroup);
                    setShowMembers(true);
                ...
                {(() => {
                  // 详细记录群主身份判断过程
                  console.log('开始判断群主身份:', {
                    currentUserId: currentUser?._id,
                    groupMembers: selectedGroup.members,
                  });

                  const isOwner = selectedGroup.members?.some(member => {
                    console.log('检查成员:', {
                      memberId: member.user,
                      memberRole: member.role,
                      currentUserId: currentUser?._id,
                      isMatch: member.user === currentUser?._id,
                      isOwner: member.role === 'owner'
                    });
                    return member.user === currentUser?._id && member.role === 'owner';
                  });

                  console.log('群主身份判断结果:', isOwner);
                  ...
                  return isOwner && (
                    <>
                      ...
                      <button 
                        ...
                        onClick={() => {
                          console.log('点击解散群聊按钮');
                          handleDisbandGroup(selectedGroup._id);
                        }}
                      >
                        解散群聊
                      </button>
                ...
                {(() => {
                  ...
                  const isMember = selectedGroup.members?.some(member => {
                    console.log('检查成员:', {
                      ...
                      currentUserId: currentUser?._id,
                      isMatch: member.user === currentUser?._id,
                      isNotOwner: member.role !== 'owner'
                    });
                    return member.user === currentUser?._id && member.role !== 'owner';
                  });

                  console.log('普通成员身份判断结果:', isMember);

                  return isMember && (
                    <button 
                      className="header-button leave-button"
                      onClick={() => {
                        console.log('点击退出群聊按钮');
              ...
              {groupMembers.map(member => {
                ...
                return (
                  <div key={member.user._id} className="member-item">
                    ...
                    {(() => {
                      // 详细记录移除成员权限判断过程
                      console.log('判断是否显示移除按钮:', {
                        currentUserId: currentUser?._id,
                        memberUserId: member.user._id,
                        isCurrentUserOwner: selectedGroup.members.some(m => 
