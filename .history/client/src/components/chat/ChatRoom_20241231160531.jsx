...
const ChatRoom = () => {
  ...
  const handleCreateGroup = async (e) => {
    ...
    try {
      const response = await fetch(api.groups.base, {
        ...
        body: JSON.stringify(groupFormData) // 发送群组创建请求
      });
      ...
      if (response.ok) {
        ...
        handleError(data.message || '创建群组失败'); // 显示错误消息
      }
    } catch (error) {
  ...
  const fetchPendingGroups = async () => {
    try {
      ...
      console.error('获取待审核群组申请失败:', error);
      handleError('获取待审核群组申请失败');
    }
  };
              ...
              {availableGroups.length > 0 ? (
                availableGroups.map(group => (
                  <div key={group._id} className="available-group-item">
                    ...
                    {pendingGroups.some(pg => pg.group === group._id) ? (
                      <span className="pending-status">审核中</span>
                    ) : (
                ...
                <div className="no-groups-message">暂无可加入的群组</div>
              )}
            </div>
            <div className="modal-footer">
              <button 
