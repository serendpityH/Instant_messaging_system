...
const KeyManagement = () => {
  ...
  const importKeys = (event) => {
    ...
    reader.onload = (e) => {
      try {
        const keyData = JSON.parse(e.target.result); // 解析JSON文件
        if (!keyData.privateKey) {
          setError('无效的密钥文件');
          return;
        }

        localStorage.setItem('privateKey', keyData.privateKey); // 将私钥存储到localStorage中
        setSuccess('密钥已成功导入');
        setError('');
      } catch (err) {
    ...
    <div className="key-management">
      ...
      <div className="key-actions">
        ...
        <div className="key-import">
          <button onClick={importKeys} className="key-button">
            导入密钥
          </button>
          <input
            type="file"
