...
class CryptoService {
  ...
  generateKeyPair() {
    try {
      const keyGenerator = new JSEncrypt({ default_key_size: 1024 }); // 设置密钥长度为1024位
      keyGenerator.getKey(); // 生成密钥对
      // 获取公钥和私钥
  ...
  setPrivateKey(privateKey) {
    try {
      ...
      this.myPublicKey = keyPair.getPublicKey(); // 设置自己的公钥
  ...
  setOtherPublicKey(userId, publicKey) {
    ...
    this.otherPublicKeys.set(userId, encryptor); // 设置其他用户的公钥
  }
  ...
  decryptMessage(encryptedMessage, senderPublicKey) {
    try {
      ...
      const aesKeyBase64 = decryptor.decrypt(encryptedMessage.key); // 解密AES密钥
  ...
  encryptGroupMessage(message, groupKey) {
    try {
      ...
      // 使用原始的十六进制群组密钥
      const key = CryptoJS.enc.Hex.parse(groupKey); // 将群组密钥转换为Hex格式
      const iv = CryptoJS.lib.WordArray.random(16); // 生成随机IV
    ...
    try {
      ...
      const decrypted = CryptoJS.AES.decrypt(
        ...
        key,
        {
          ...
          padding: CryptoJS.pad.Pkcs7 // 使用PKCS7填充模式
        }
      );
